...
TOKEN = "ae8d737d7c78"
MY_NUMBER = "919161511029"  # Insert your number {91}{Your number}
...
async def resume() -> str:
    try:
        # Update to your actual resume file name
        resume_path = Path("Abhijeet_Singh_AI_ML.pdf")
        ...
        if not text.strip():
            return "<error>Resume PDF has no extractable text.</error>"

        md = markdownify.markdownify(text, heading_style="ATX")
        return md.strip()

    except Exception as e:
        return f"<error>Failed to process resume: {e}</error>"
